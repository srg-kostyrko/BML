...
const { NotImplementedError, SizeError } = require('../errors');
...
class Tag {
  ...
  size() {
    const context = new Context();
    return this._size_(context);
  }

  _size_(_context) {
    throw new SizeError(`cannot calculate size of ${this.constructor.name}`);
  }
...
class Adapter extends Tag {
  ...
  _size_(context) {
    return this.tag._size_(context);
  }
